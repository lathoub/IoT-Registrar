...
import provider from '../../../models/common/collections/provider.js'
...
export function get (req, res) {
  ...
  provider.get(formatFreeUrl, format, providerId, function(err, content) {
    ...
    switch (format) {
      ...
      case `html`:
        ...
        // This recommendation does not apply, if there are a large number of links included 
        // in a response or a link is not known when the HTTP headers of the response are created.
        res.status(200).render(`provider`, content )
        break
      default:
